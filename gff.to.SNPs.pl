...
# INPUT INFO
@fileList= ('data/Gff.Head-thorax.txt', 'data/Gff.Abdomen.txt', 'data/Gff.DigestiveEnzymes.txt', 'data/Gff.SFPs.txt', 'Gff.transcriptome.txt');
...
foreach $list (@fileList)
{
	...
	@name = split(/\//,$list);
	@name = split(/\./,@name[1]);
	open (OUTFILE, ">SNPs.Gff.@name[1].txt");
	close(OUTFILE);
				...
				if ((@quiver[0] eq @quiver1[0]) && (@gff[3] <= @fst[1]) && (@gff[4] >= @fst[1]))
				{
					...
					foreach $CMH (@contigValuesCMH)
					{
						...
						if ((@quiver2[0] eq @quiver[0]) && (@cmh[1] eq @fst[1]))
						{
							...
							open (OUTFILE, ">>SNPs.Gff.@name[4].txt");
